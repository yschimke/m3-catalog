...
- **Never attribute a commit to an AI agent** — no `Co-authored-by:` trailer naming an agent, and
  ...
  `_Generated by [Claude Code]_` footer are fine; they don't claim authorship. Enforced in two
  places sharing one detector
  ...
  [`No Agent Attribution`](.github/workflows/no-agent-attribution.yml) CI gate. **The hooks alone
  were not enough, and `main` has four scars to prove it** — two commits carrying an agent identity,
  which a hook that is not installed or is bypassed with `--no-verify` never saw, and two
  `Co-authored-by:` trailers that no local hook could have stopped, because GitHub builds the squash
  message from the PR title and body and credits every distinct branch commit author, all of it
  server-side. So scrub the **PR description** too, not just the commits. The gate only *blocks*
  once it is a required status check on the Protect Main ruleset; until then its `drift` job makes a
  breach loud rather than preventing it.
- **Reviewers, human or bot: verify before reporting an attribution finding.** It is the
  ...
  Report only what it exits 1 on, and quote its output. `Yuri Schimke <yuri@schimke.ee>` is this
  repository's human identity, and `github-actions[bot]` / `renovate[bot]` are exempt bot accounts —
  none of the three is ever a finding. If the `Reject agent attribution` check is already green on
  the head commit, there is nothing to report.

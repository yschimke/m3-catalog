...
samples declare — `androidx/compose/material3/samples/Foo.kt`, not `Foo.kt`. `applyPatches` hands
the whole tree to `git apply --directory`, and a path missing the package fails the import with
"upstream has moved under this patch", which is the one thing it would not mean.
...
# edit /tmp/samples-fresh/androidx/compose/material3/samples/Foo.kt

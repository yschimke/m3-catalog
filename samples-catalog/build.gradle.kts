// `:samples-catalog` — the AndroidX material3 samples, rendered.
//
...
// package (`androidx.compose.material3.samples`), fetched by `scripts/import-samples.mjs` from the
// commit pinned in `samples/import.json`. It is never edited in place and never formatted — a fix
// is a patch in `samples/patches/` with a stated reason, so the next import re-applies it and a
// patch that stops applying fails the import instead of silently vanishing.
//
// The samples already carry `@Preview` upstream — 298 of the 317 `@Sampled` functions do — so
// discovery finds them directly and no wrapper is generated for them. What they cannot declare for
// themselves is a theme, so `catalog.spec.json` supplies one through its `themes` block, which
// exists precisely for an imported project.
plugins {
...
dependencies {
  ...
  // `currentWindowAdaptiveInfo` / `WindowSizeClass`, which the app-bar samples call. The same
  // adaptive artifacts `:catalog` carries.

...
abstract class GenerateRuntimePolicy : DefaultTask() {
  ...
  fun generate() {
    ...
    outputDirectory
      .get()
      .file("ee/schimke/m3catalog/uibuilder/GeneratedRuntimePolicy.kt")
      .asFile
      .apply {
        parentFile.mkdirs()
        writeText(
...
val runtimeIdentity =
  providers
    ...
    .map { "m3-catalog-p2-${it.take(12)}" }
    .orElse("m3-catalog-p2-development")
...
kotlin {
  ...
  wasmJs {
    browser()
    outputModuleName.set("m3CatalogRenderer")
    binaries.executable()
  }
...
val runtimeAssets =
  tasks.register<Sync>("runtimeAssets") {
    ...
    // Skiko's browser loader and the renderer font resolver consume these at runtime. They are
...
val wasmRendererDist =
  tasks.register<AssembleCatalogRendererRuntime>("wasmRendererDist") {
    description = "Assemble the transitional catalog-owned Material 3 UI-builder renderer."
    group = "distribution"
...
val rendererArchive =
  tasks.register<Zip>("rendererArchive") {
    description = "Package the immutable catalog-owned Material 3 UI-builder renderer."
    group = "distribution"
    dependsOn(wasmRendererDist)
    from(wasmRendererDist.flatMap { it.outputDirectory })
    archiveFileName.set("m3-catalog-ui-builder-renderer.zip")
    ...
    ZipFile(archiveFile.get().asFile).use { zip ->
      ...
      val required =
        setOf(
          ...
          "index.html",
          "m3CatalogRenderer.mjs",
          "m3CatalogRenderer.wasm",
          "skiko.mjs",
          "skiko.wasm",
        )

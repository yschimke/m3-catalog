...
@OverrideVariant(name = "s-narrow-square", strings = ["width=narrow", "shape=square"])
@OverrideVariant(name = "s-square", strings = ["shape=square"])
@OverrideVariant(name = "s-wide", strings = ["width=wide"])
@OverrideVariant(name = "s-wide-square", strings = ["width=wide", "shape=square"])
...
/**
 * The **type scale matrix**: the fifteen roles of [TypeScaleRole], fourteen cells beside the base
 * render.
 *
 * One axis and no cross product, so the cells are the roles themselves. They were hand-typed beside
 * a `when` over the same fifteen literals and a second list of display labels, with nothing
 * relating the three (#103); the enum now carries all of it.
 */
...
annotation class TypeScaleMatrix

/**
 * The **corner scale matrix**: the ten radius tokens of [CornerScaleToken], nine cells beside the
 * base render.
 *
 * `none` is the base — an unseeded specimen draws the unrounded box — so the nine cells are the
 * tokens that round it, up to the 50% corner `full` names.
 */
...
annotation class CornerScaleMatrix

/**
 * The **colour mode matrix**: the six schemes of [CatalogSchemeChoice], five cells beside the base
 * render.
 *
 * Light and dark at standard, medium and high contrast — the kit's own six modes. The base render
 * is the standard light scheme, so the cells are the other five.
 */
...
annotation class ColorSchemeMatrix

...
test("a sample vendors into the directories its package names", () => {
  ...
  const { root, library } = fakeUpstream({ "ButtonSamples.kt": "fun a() {}" });
  const out = mkdtempSync(join(tmpdir(), "out-"));
  try {
    const [copied] = vendor(root, library, out).copied;
...
test("the package directory is whatever follows the module's source root", () => {
  assert.equal(
    packageDirOf("compose/material3/material3/samples/src/main/java/androidx/x/samples"),
    "androidx/x/samples",
  );
...
test("a file name vendored by two libraries fails the import rather than picking a winner", () => {
  // `samples/quarantine.json` keys on the bare file name, so a collision would quietly widen a
  // quarantine entry to a file its author never named. The package directories keep the two from
  // overwriting each other on disk, which is exactly why the check asks about the NAME: a path
  // check would pass on the case that breaks the invariant.
  const { root, library } = fakeUpstream({ "A.kt": "fun a() {}" });
  const out = mkdtempSync(join(tmpdir(), "out-"));
  try {
    vendor(root, library, out);

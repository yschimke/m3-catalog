...
#
# The step resolves every authored prop that uses the kit's vocabulary and reports the remainder.
# The reason for keeping the report visible is worth stating:
#
#  * A prop such as `size=Large` resolves by changing that axis from the referenced base cell. The
#    generated exhaustive view used by `m3-catalog` is still needed to publish every kit cell.
#  * Most unmatched props have NO kit counterpart to find. `content=leading-icon` is a Compose slot;
...
#
# Some unmatched props have a counterpart but use Compose's vocabulary: ToggleButton `state=checked` ->
...
# kit's `Type=2-line` (`384:4191`) under a Compose name. Renaming those would make the catalog less
# readable; the generated exhaustive view is where the exact kit vectors belong.
#
# ## --prefix
#
